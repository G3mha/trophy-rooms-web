...
function SelectTrigger({
  ...
  return (
    <SelectPrimitive.Trigger
      ...
      className={cn(
        "flex h-10 w-full items-center justify-between gap-2 rounded-[var(--border-radius)] border-2 border-[var(--border-color)] bg-[var(--bg-secondary)] px-4 py-2 text-base text-[var(--text-primary)] transition-colors outline-none focus:border-[var(--nintendo-red)] focus:ring-2 focus:ring-[var(--nintendo-red)]/20 disabled:cursor-not-allowed disabled:opacity-50 data-placeholder:text-[var(--text-muted)]",
        className
      )}
      {...props}
    >
    ...
    <SelectPrimitive.Portal>
      ...
      >
        <SelectPrimitive.Popup
          ...
          className={cn(
            "relative isolate z-50 max-h-[var(--available-height)] w-[var(--anchor-width)] min-w-36 overflow-x-hidden overflow-y-auto rounded-[var(--border-radius)] border border-[var(--border-color)] bg-[var(--bg-card)] text-[var(--text-primary)] shadow-lg duration-100 data-open:animate-in data-open:fade-in-0 data-open:zoom-in-95 data-closed:animate-out data-closed:fade-out-0 data-closed:zoom-out-95",
            className
          )}
          {...props}
        >
